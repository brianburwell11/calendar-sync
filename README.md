...
| `color` | optional event color. Pick a name from the dropdown or type a number `1`–`11`. Blank = the destination calendar's default color. `full`/`busy`: colors the copy. `invite`: colors **your own copy** on the Destination, but only for events this mapping invited and only if you have edit access there (the organizer and other guests are unaffected; clearing the color later leaves already-colored events as-is). Accepted names: `Lavender` (1), `Sage` (2), `Grape` (3), `Flamingo` (4), `Banana` (5), `Tangerine` (6), `Peacock` (7), `Graphite` (8), `Blueberry` (9), `Basil` (10), `Tomato` (11). |
